...
We use a "jump box" (Jump-Box-Provisioner) as a gateway between the host machine (Client/User) and our webservers (Web-1 through -3), and the host machine and the ELK server (VM-4). 
...
### Configuring Files and Using Playbooks
## Configuring and Installing ELK Server with Docker
...
We added the internal IP of our new VM-4 to the Ansible "hosts" file in our container "elastic_blackburn" located in the Jump-Box-Provisioner (see step 4 under Filebeat and Metricbeat Installation below). We then created a playbook to use with the new VM-4 (ELK) machine, specifying a different group of machines and the remote use. We increase its memory, and included the installation of `docker.io`, `python3-pip`, and `docker`. Here is the complete playbook [Config and Install ELK with Docker](Config&installElk)
...
[Elk Container Installed](Elk Launched)

A security rule was created on port `5601` to allow TCP traffic from the Client/User IP address into the new VM-4 Elk server. We verify connectivity by visiting `http://[VM-4.IP]:5601/app/kibana`, using the public address of the new VM-4 machine.

## Configuring and Installing Monitoring Services
To configure files to set Filebeat and Metricbeat services, we follow these steps:

1. Open the terminal and access jump-box. In this case, we SSH to our Jump-Box-Provisioner.
2. Start Ansible container. In this case, we run `sudo docker start elastic_blackburn && sudo docker attach elastic_blackburn`
...
5. Navigate to `http://[VM-4.IP]/app/kibana` using the public IP address of the Elk Server (in our case, VM-4).
